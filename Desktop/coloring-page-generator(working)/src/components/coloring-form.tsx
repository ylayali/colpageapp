...
import { Loader2, User, Palette, Activity, Upload, X } from 'lucide-react';
...
export function ColoringForm({ onSubmit, isLoading, userCredits }: ColoringFormProps) {
    ...
    const [sourceImagePreviews, setSourceImagePreviews] = React.useState<string[]>([]);
    ...
    const handleSourceImagesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files) {
            const files = Array.from(event.target.files);
            setSourceImages(prev => [...prev, ...files]);
            const newPreviews = files.map(file => URL.createObjectURL(file));
            setSourceImagePreviews(prev => [...prev, ...newPreviews]);
        }
    };

    const removeSourceImage = (index: number) => {
        setSourceImages(prev => prev.filter((_, i) => i !== index));
        setSourceImagePreviews(prev => {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (coloringPageType !== 'cartoon_portrait' && !photo) return;
        if (coloringPageType === 'cartoon_portrait' && sourceImages.length === 0) return;
    ...
    React.useEffect(() => {
        return () => {
            ...
            sourceImagePreviews.forEach(url => URL.revokeObjectURL(url));
        };
    }, [photoPreview, sourceImagePreviews]);

    const showBackgroundOption = coloringPageType !== 'straight_copy';
    const showActivityOption = coloringPageType === 'cartoon_portrait';
    const canSubmit = (photo || sourceImages.length > 0) && userCredits > 0 && !isLoading;
                        ...
                        <div className='space-y-3 rounded-md border border-white/20 bg-black/20 p-4'>
                            ...
                            <div className='space-y-2'>
                                ...
                                <div className='grid grid-cols-3 gap-2'>
                                    {sourceImagePreviews.map((previewUrl, index) => (
